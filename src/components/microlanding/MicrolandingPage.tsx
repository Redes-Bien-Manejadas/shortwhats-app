...
import { RedirectOverlay } from '@/components/redirect/RedirectOverlay';
...
export function MicrolandingPage({ linkData }: { linkData: LinkData }) {
  ...
  const handleWhatsAppClick = (e: React.MouseEvent) => {
    e.preventDefault(); // Prevent navigation to /redirect/[slug]

    // Increment click count (server handles debounce)
    fetch(`/api/links/${linkData.slug}/clicks`, { method: 'POST' }).catch(() => { });

    // Show redirect overlay inline instead of navigating
    setShowRedirectOverlay(true);
  };

  return (
    <>
      {/* Facebook Pixel: PageView + ViewContent on page load, Lead fires in overlay after validation */}
      <FacebookPixel config={facebookPixel} onPageView={true} />

      {/* Redirect overlay - shown inline when user clicks WhatsApp button */}
      {showRedirectOverlay && (
        <RedirectOverlay
        ...
        <main className="w-full max-w-2xl mx-auto text-center px-4">
          ...
          <div className="mb-10">
            <Button
              ...
              style={buttonStyle}
            >
              <a href={`/redirect/${linkData.slug}`} onClick={handleWhatsAppClick} className="flex items-center justify-center">
