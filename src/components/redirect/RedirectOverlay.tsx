...
function RedirectOverlayContent({ targetUrl, slug, message, facebookPixel, onClose }: RedirectOverlayProps) {
    ...
    // Fire Lead event only after validation succeeds
    const fireLeadEvent = useCallback(() => {
        if (leadFired.current) return;
        if (!facebookPixel?.pixelId || !facebookPixel?.leadEvent) return;
    ...
    // Fire Lead when validation succeeds
    useEffect(() => {
        if (isValidated) {
            fireLeadEvent();
        }
    }, [isValidated, fireLeadEvent]);
    ...
    const validateContact = useCallback(async () => {
        ...
        try {
            ...
            if (response.ok && data.success) {
                setIsValidated(true);
                console.log('✅ Contact validated, Lead event can now fire');
    ...
    const handleClose = () => {
        if (onClose && (validationError?.includes('Demasiados') || isBotDetected)) {
            onClose();
        }
    };
    ...
    return (
        <div className="fixed inset-0 z-50">
            {/* Show rate limit error if blocked */}
            {validationError && validationError.includes('Demasiados') && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50">
                    <div className="bg-white rounded-lg p-6 max-w-sm mx-4 text-center">
                        <p className="text-red-600 font-semibold mb-2">⚠️ Demasiados intentos</p>
                        <p className="text-gray-600 mb-4">Por favor espera unos minutos antes de intentar nuevamente.</p>
                        {onClose && (
            ...
            {isBotDetected && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50">
                    <div className="bg-white rounded-lg p-6 max-w-sm mx-4 text-center">
                        ...
                        <p className="text-gray-600 mb-4">No pudimos verificar que eres una persona real. Por favor intenta de nuevo.</p>
                        {onClose && (
                            <Button onClick={handleClose} variant="outline" className="mt-2">
                                Volver
                            </Button>
...
export function RedirectOverlay(props: RedirectOverlayProps) {
    return (
        <RecaptchaProvider>
            <RedirectOverlayContent {...props} />
        </RecaptchaProvider>
    );
}
